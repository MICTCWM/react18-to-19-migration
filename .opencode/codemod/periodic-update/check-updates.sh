#!/bin/sh
set -eu
STATE_FILE='F:\code\dorahacks\react18-to-19-migration\.opencode\codemod/periodic-update\last-check-epoch-secs'
INTERVAL=21600
...
OUTPUT="$(
{
if [ -x 'F:\code\dorahacks\react18-to-19-migration\node_modules\@codemod.com\cli-win32-x64-msvc\codemod.exe' ]; then
  'F:\code\dorahacks\react18-to-19-migration\node_modules\@codemod.com\cli-win32-x64-msvc\codemod.exe' 'ai' '--harness' 'opencode' '--project' '--update-policy' 'auto-safe' '--require-signed-manifest' '--format' 'json'
  exit $?
fi
